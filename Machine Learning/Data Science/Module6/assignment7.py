...
#
lab = ['sample', 'thickness', 'size', 'shape', 'adhesion', 'epithelial', 'nuclei', 'chromatin', 'nucleoli', 'mitoses', 'status']
X = pd.read_csv('Datasets/breast-cancer-wisconsin.data',header = None,names = lab)
X = X.dropna(axis = 0)
# 
...
#
status = X['status']
X = X.drop(labels = ['sample','status'],axis = 1)
#print(status)
#print(X.dtypes)
#
...
#
status = status.fillna(status.mean())
